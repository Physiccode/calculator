...
#[derive(Clone,Copy)]
pub enum Root{  //complex number:Z=a+bi,Re(z)=a,Im(z)=b
  Real(f64),
...
fn check_precision(x:f64)->f64{
  ...
  if x > 0.0 {
    ...
    if (1.0-decimal) < EPS{  //if the decimal part is very close to 1,we return ceil(x)

      return x.ceil();

    }
    else {x}  //otherwise,we just return x
  }
  ...
  fn extract(&self)->String{
    ...
    match self {
      ...
      Root::Complex{re,im}=>
      {
        ...
        if *re < EPS && *im < EPS{  //if they are both are = 0
          format!("0")
        }
        else if *re < EPS {  //if the real part is = 0
          format!("{}i",im)
        }
        else if *im < EPS {  //if the imaginary part is = 0
          format!("{}",re)
        }
        else {
          if *im < 0.0 {
...
impl Roots for Depressed_cube{
  fn get_roots(&self)->DepressedRoots{
    const EPS:f64 = 1e-12;  //Numerical precision
    let delta     = self.discriminant();

    if delta > 0.0 {  //1 real root,2 complex conjugate roots
      ...
      let y_2 = Root::Complex{re:first_part,im:last_part};
      let y_3 = Root::Complex{re:first_part,im:-last_part};
    ...
    else if delta.abs() < EPS  { // if the discriminant is equal to 0:three real roots,at least two are equal
      if self.p == self.q && self.p == 0.0  { //if this condition is true,then all roots must be equal to 0
        let r = Root::Real(0.0);

        DepressedRoots{y_1:r,y_2:r,y_3:r,a:self.a}
      }

      else {  //then,2 roots are equal
    ...
    else if delta < -EPS {  //three distinct real roots,we must switch to trigonometry
      ...
      panic!("unknown error,perhaps,did you make sure you're following the correct cubic form\ndelta={}",delta);
    }
  }
}
...
impl Original for DepressedRoots{
  fn get_original_root_y_1(&self)->Root{  //plug back x
    match self.y_1{

      Root::Real(r)=>{
        Root::Real(check_precision(r-(self.a/3.0)))
      },
